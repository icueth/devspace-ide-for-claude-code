...
const SKILL_BEGIN = '<!-- BEGIN devspace-codeflow-skill (auto-managed) -->';
const SKILL_END = '<!-- END devspace-codeflow-skill -->';
...
async function writeSkill(projectPath: string): Promise<void> {
  ...
  const body = [
    ...
    '3. Quote the doc\'s `path:line` references when explaining — they were generated against the actual files in this repo.',
    '4. If the docs feel out of date, mention that the user should run **DevSpace → Codeflow → Re-analyze** to refresh.',
    '',
    '# Notes',
    '',
